...
from attractor import compute_conductance_scaling, compute_conductance_scaling_single_clip
...
class TestClassFctComputeConductanceScalingSingleClip(TestCase):
    @staticmethod
    def _naive_compute_conductance_single_clip(patterns:np.ndarray, sparsity:float, g_ee:float, c_ee:float):
        size = patterns.shape[1]
        ...
        for i in range(size):
            for j in range(size):
                for p in range(patterns.shape[0]):
                    g[i][j] += g_ee / c_ee * (patterns[p][i] / sparsity - 1) * (patterns[p][j] / sparsity - 1)
                g[i][j] = max(0, g[i][j])
        return g
    ...
    def test_when_called_should_produce_similar_result_to_naiv_impl_when_scaled(self):
        ...
        g_should = self.__class__._naive_compute_conductance_single_clip(patterns, sparsity, g_ee, c_ee)
