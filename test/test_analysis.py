...
class TestClassExperimentAnalaysis(TestCase):
    ...
    def test_instaneous_rate_when_spikes_occur_rate_should_be_a_multiple_of_count_with_factor_dependent_on_dt_and_pop_size(self):
        with TestEnv():
            f_name = "file.h5"
            with BrianExperiment(persist=True, path=f_name) as exp:
                ...
                E = NeuronPopulation(pop_size, eqs_P, threshold='v_s>-30*mV', refractory=1.3*ms, method='rk4')
                E.monitor_rate()
                E.monitor(E.ids, ["v_s"])
...
class TestFunctionGaussianSmoothing(TestCase):
    def test_gaussian_smoothing_when_computing_smooth_rate_should_yield_same_result_as_brian2_smoothing_fct(self):
        ...
        with TestEnv():
            ...
            computed_rate = gaussian_smoothing(instantaneous_rate=inst_rate, window_size=2.0, one_sigma_window=1.0, dt=dt)
