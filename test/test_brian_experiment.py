...
class TestClassBrianExperiment(TestCase):
    ...
    def test_when_persist_true_and_no_experiment_folder_should_create_folder_and_autogenerate_file_name_and_persist(
        self,
    ):
        with BrianExperiment(persist=True) as exp:
            self.assertEqual(exp.path, os.path.abspath("./experiments/exp_0.h5"))

    def test_when_persist_true_should_auto_generate_file_bane_in_sequence_and_persist(
        self,
    ):
    ...
    def test_when_persist_not_set_and_object_path_provided_should_raise_value_error(
        self,
    ):
        with self.assertRaises(ValueError):
            BrianExperiment(object_path="/bla")

    def test_when_persist_set_and_incorrect_object_path_provided_should_raise_opath_error(
        self,
    ):
    ...
    def test_member_persist_data_when_populating_should_write_added_entries_to_file_on_exit(
        self,
    ):
    ...
    def test_member_persist_data_when_providing_object_path_should_write_added_entries_to_file_attaching_nodes_below_object_path_on_exit(
        self,
    ):
        with BrianExperiment(
            persist=True, path="file.h5", object_path="/run_1/data"
        ) as exp:
            exp.persist_data["vals"] = np.arange(200)

        with FileMap(path="file.h5", mode="read") as fm:
            self.assertTrue(
                np.all(fm["run_1"]["data"]["persist_data"]["vals"] == np.arange(200))
            )
    ...
    def test_member_persist_data_when_nested_dict_assigned_should_convert_to_nested_persisted_data_class(
        self,
    ):
        with BrianExperiment(persist=True) as exp:
            exp.persist_data["data"] = {"a": {"b": "c"}}
            self.assertEqual(
                exp.persist_data["data"].__class__, BrianExperiment.PersistData
            )

    def test_member_persist_data_when_reassigned_should_fail_as_is_read_only_property(
        self,
    ):
        with BrianExperiment(persist=True) as exp:
    ...
    def test_when_persist_set_should_auto_save_instances_of_neuron_population(self):
        ...
        with FileMap(path="file.h5", mode="read") as fm:
            ...
            self.assertTrue(
                np.all(
                    fm[SpikeDeviceGroup.__name__]["E"]["state"]["v"]
                    == available_data["state"]["v"]
                )
            )
            self.assertTrue(
                np.all(
                    fm[SpikeDeviceGroup.__name__]["E"]["state"]["t"]["value"]
                    == np.array(available_data["state"]["t"]["value"])
                )
            )
    ...
    def test_when_persist_set_should_auto_save_instances_of_synapse(self):
        ...
        with FileMap(path="file.h5", mode="read") as fm:
            self.assertTrue(
                np.all(
                    fm[Synapse.__name__]["syn_ee"]["state"]["x_AMPA"]
                    == available_data["state"]["x_AMPA"]
                )
            )
            self.assertTrue(
                np.all(
                    fm[Synapse.__name__]["syn_ee"]["state"]["t"]
                    == np.array(available_data["state"]["t"])
                )
            )
    ...
    def test_method_retrieve_callers_context_when_called_from_two_separate_instances_should_return_same_context(
        self,
    ):
        exp = BrianExperiment()
        ...
        y = 4
        self.assertEqual(
            exp._retrieve_callers_context(), exp_._retrieve_callers_context()
        )
    ...
    def test_method_retrieve_callers_context_when_called_with_different_stacks_should_return_same_context(
        self,
    ):
        exp = BrianExperiment()
        ...
        self.assertEqual(
            exp._retrieve_callers_context(),
            {
                k: v
                for k, v in [*exp._retrieve_callers_frame().frame.f_globals.items()]
                + [*exp._retrieve_callers_frame().frame.f_locals.items()]
            },
        )

    def test_method_retrieve_callers_context_when_multiple_brian_experiments_are_done_should_have_separate_context(
        self,
    ):
    ...
    def test_method_collect_devices_when_called_should_collect_wrapped_brian_objects_created_in_experiment_scope(
        self,
    ):
        with BrianExperiment() as exp:
            E = NeuronPopulation(
            ...
            self.assertTrue(
                len(devices) == len(devices_should)
                and all([dev in devices_should for dev in devices])
            )

    def test_when_get_namespace_called_should_get_brian_namespace_which_is_module_in_neuron_param_module_class_var_plus_global_scope_of_calling_scope(
        self,
    ):
        with BrianExperiment() as exp:
            ...
            should_diff, is_diff = compare_iterables(
                namespace_should.keys(), namespace.keys()
            )
            # raise ValueError(f"should: {''}  - is: {''}. - should_diff{should_diff}, is_diff{is_diff}")

            self.assertTrue(
                all([ns in namespace.keys() for ns in namespace_should.keys()])
            )
