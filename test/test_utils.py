...
class TestFctConvertAndCleanBrian2Quantity(TestCase):
    def test_when_called_should_return_base_unit(self):
        x = np.arange(10) * ms
        _, unit = convert_and_clean_brian2_quantity(x)
        self.assertEqual(unit, str(second))

    def test_when_quantity_in_non_base_unit_passed_should_return_same_values_as_provided_scaled_with_unit_scale_factor_relative_to_base_unit(
        self,
    ):
        x = np.arange(10) * ms
        y, _ = convert_and_clean_brian2_quantity(x)
        self.assertTrue(np.allclose(x / ms / 1000, y))

    def test_when_nested_quantity_of_size_one_passed_should_return_np_ndarray(self):
        x = np.array([0]) * ms
        y, _ = convert_and_clean_brian2_quantity(x)
        self.assertTrue(isinstance(y, np.ndarray))

    def test_when_non_nested_quantity_of_size_one_passed_should_return_scalar_value(
        self,
    ):
        x = np.array(0, dtype=float) * ms
        y, _ = convert_and_clean_brian2_quantity(x)
        # raise ValueError(y,type(y))
...
class TestFctUniqueIdx(TestCase):
    def test_when_called_with_tiled_input_should_return_unique_indices(self):
        ...
        self.assertTrue(
            np.all(
                np.asarray(
                    [
                        np.sum(y[idx]) / (i) if i > 0 else float(len(y[idx]))
                        for i, idx in enumerate(unique_idx(y)[1])
                    ]
                )
                == float(repeat)
            )
    ...
    def test_when_called_with_arbitrary_input_should_return_unique_indices(self):
        ...
        should_idx = [
            np.asarray(e) for e in [[0, 1, 5, 7], [6], [2], [3, 9], [4, 8, 10]]
        ]
        ...
        self.assertTrue(
            all(
                [
                    np.all(is_ == should_)
                    for is_, should_ in zip(unique_idx(x)[1], should_idx)
                ]
            )
        )
...
class TestFctValuesInInterval(TestCase):
    def test_when_duration_divided_by_time_step_has_no_remainder_should_return_correct_number_of_values(
        self,
    ):
        t0 = 1.0
        t1 = 11.0
    ...
    def test_when_duration_divided_by_time_step_has_remainder_should_return_correct_number_of_values(
        self,
    ):
        t0 = 1.0
        t1 = 11.0
...
class TestFunctionRestrictToInterval(TestCase):
    ...
    def test_when_upper_bound_exceeds_simulation_time_should_return_only_values_until_the_end_and_the_final_simulation_time(
        self,
    ):
        dt = 0.01
        t = 100.0
